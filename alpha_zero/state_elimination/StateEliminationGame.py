...
#0 as emptyness, 5 + max(ALPHABET) should be the input size of embedding dimension
word_to_ix = {'@': 1, '+': 2, '*': 3, '.': 4}
...
class StateEliminationGame():
    ...
    def gfa_to_tensor(self, gfa):
        ...
        for i in range(action_size):
            #여기 원 핫 벡터 말고 단순하게 숫자만 넣어서 임베딩하는게 깔끔해보임
            if i < len(gfa.States):
            ...
            '''
            print()
            print(f"source_state_number: {len(source_state_number)}")
            print(f"is_initial_state: {is_initial_state}")
            print(f"is_initial_state: {is_initial_state}")
            print(f"in_transition: {len(in_transition)}")
            print(f"out_transition: {len(out_transition)}")
            exit()
            #'''
        return tensor
