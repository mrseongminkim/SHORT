...
from alpha_zero.set_transformer.blocks import ISAB, PMA, SAB
...
class SetTransformer(nn.Module):
    def __init__(self, in_dim, out_dim=None, embed_type="invariant"):
        ...
        self.value_head = nn.Linear(k * d, out_dim)

        self.policy_head = nn.Linear(d, out_dim)
    ...
    def forward(self, x):
        ...
        b, k, d = x_dec.shape
        value = self.value_head(x_dec.view(b, k * d))

        policy = self.policy_head(x_enc)
