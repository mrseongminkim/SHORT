...
annotation_file = open("./valid_data2/annotations_file.csv", "w", newline="")
writer = csv.writer(annotation_file)
count = 0
data_size = 100#_000 #80_000 for train and 20_000 for valid
progress = tqdm.tqdm(total=data_size)

while count != data_size:
    gfa: GFA = game.get_initial_gfa(n=6, k=5, d=0.1)
    ...
    while game.getGameEnded(gfa) == None:
        ...
        for i in range(len(gfa.States)):
            ...
            if i != gfa.Initial and i not in gfa.Final and weight < min_weight:
                min_weight = weight
                min_idx = i
        with open(f"./valid_data2/gfa{count}_step{step}.pkl", "wb") as fp:
            pickle.dump(gfa, fp)
